...
class BoardWriteActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding.writeBtn.setOnClickListener {
            ...
            // 파이어베이스 storeg에 이미지를 저장하고 싶은데
            // 만약에 내가 게시글을 클릭했을 때, 게시글에 대한 정보를 받아와야 하는데
            // 이미지 이름에 대한 정보를 모르기 때문에
            // 이미지 이름을 문서의 key값으로 해줘서 이미지에 대한 정보를 찾기 쉽게 해줘야한다

            val key = FBRef.boardRef.push().key.toString()      // key값을 먼저 생성한뒤에 데이터베이스에 저장할 수 있다

            FBRef.boardRef
                .child(key)                                    // 이전에는 push()-> key값을 데이터베이스에서 임의로 생성
